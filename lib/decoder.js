...
EiDecoder.prototype.skipTerm = function() {
    ...
    switch ((type = this.buf.readUInt8(this.index))) {
        ...
        case ei.LIST_EXT:
            cnt = this.decodeListHeader();
            while (cnt-- && this.skipTerm()) {}
            if (ei.NIL_EXT === this.getType())
                this.skipTerm();
            else
                throw new Error('LIST_EXT not followed by NIL_EXT');
            break;
