...
EiEncoder.prototype._extend = function(len) {
    if ((len + (this.index - this.offset)) > this.buf.length) {
        //console.log('EXTEND: (at=%d, off=%d, len=%d)', this.index, this.offset, len);

        this.buffers.push(this.buf.slice(this.offset, this.index));
        this.buf = new Buffer(Math.max(len, BUFFER_SIZE));
        //this.buf.fill(0);
        this.index = 0;
        this.offset = 0;
    }
...
EiEncoder.prototype.encodePid = function(pid) {
    this.appendUInt8(ei.PID_EXT);
    this.encodeAtom(pid.node);
    this.appendUInt32BE(pid.number & 0x7fff); // 15
    this.appendUInt32BE(pid.serial & 0x1fff); // 13
    this.appendUInt8(pid.creation & 0x03);    // 2
};

EiEncoder.prototype.encodeRef = function(ref) {
    this.appendUInt8(ei.NEW_REFERENCE_EXT);
    this.appendUInt16BE(ref.len);
    this.encodeAtom(ref.node);
    this.appendUInt8(ref.creation & 0x03);

    for (var i=0; i<ref.len; i++) {
        this.appendUInt32BE(ref.n[i]);
    }
};
...
EiEncoder.prototype.appendBuffer = function(val, len) {
    ...
    if (0 < len) {
        this._extend(len);
        val.copy(this.buf, this.index, 0, len);
        this.index += len;
    }
}
